-- KAL STUDIO — COMPLETE DATABASE SETUP
...
-- Function to create user profile after auth signup (with ON CONFLICT handling)
...
-- Function to get user's assigned projects (for employee dashboard)
...
DO $$
BEGIN
    RAISE NOTICE '✅ Kal Studio database setup completed successfully!';
    ...
    RAISE NOTICE '   1. Create your owner account through the application';
    RAISE NOTICE '   2. Or insert an owner user manually:';
    RAISE NOTICE '      INSERT INTO public.users (id, email, name, role, status)';
    RAISE NOTICE '      VALUES (''your-auth-user-id'', ''owner@example.com'', ''Owner'', ''owner'', ''active'');';
END $$;