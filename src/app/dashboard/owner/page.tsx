...
export default function OwnerDashboard() {
  const [data, setData] = useState<DashboardData>({
    gigs: [],
    metrics: { revenue: 0, profit: 0, active: 0, payouts: 0 },
  });
    ...
    async function fetchDashboardData() {
      try {
        ...
        const { data: gigs, error: gigsError } = await supabase
          .from("gigs")
          .select(
            "*, clients(name, business), payments(amount_received), payouts(amount, is_paid)",
          )
        ...
        const revenue =
          allPayments?.reduce((acc, p) => acc + Number(p.amount_received), 0) ||
          0;
        const settledPayouts =
          allPayouts?.reduce((acc, p) => acc + Number(p.amount), 0) || 0;
        const pending =
          pendingPayouts?.reduce((acc, p) => acc + Number(p.amount), 0) || 0;
  ...
  if (error)
    return (
      <div className="p-32 flex justify-center text-red-500 font-bold">
        {error}
      </div>
    );
    ...
    <div className="space-y-16 animate-in fade-in duration-700 slide-in-from-bottom-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-brand-charcoal/10 pb-10">
        ...
        <div className="max-w-[200px] text-[11px] text-brand-charcoal/40 font-medium leading-relaxed italic text-right md:text-left">
          &ldquo;The scale of your vision determines the height of your architecture.&rdquo;
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-l border-t border-brand-charcoal/10">
        ...
        <div className="border-r border-b border-brand-charcoal/10">
          <MetricCard
            ...
            value={`₹${data.metrics.payouts.toLocaleString()}`}
            trend="Due"
            trendUp={false}
            icon={Users}
          />
        ...
        <div className="space-y-10">
          <div className="flex justify-between items-baseline border-b border-brand-charcoal/10 pb-4">
            ...
            <Link
              ...
              className="text-[10px] font-bold text-brand-gold hover:text-brand-charcoal uppercase tracking-widest transition-colors flex items-center gap-1"
            >
          ...
          <div className="space-y-0 border-t border-brand-charcoal/5">
            {data.gigs.length > 0 ? (
               data.gigs.map((gig) => (
                <div
                  key={gig.id}
                  className="grid grid-cols-[2fr_1fr_1fr] items-center py-8 border-b border-brand-charcoal/10 group hover:bg-brand-charcoal/[0.02] transition-colors px-4"
                >
                  ...
                  <div className="flex justify-center">
                    <span
                      className={`text-[9px] font-bold uppercase tracking-widest px-3 py-1 border whitespace-nowrap ${
              ...
              <p className="py-20 text-center text-xs font-bold uppercase tracking-widest text-brand-charcoal/20">
                No projects record yet
              </p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-10">
          <div className="flex justify-between items-baseline border-b border-brand-charcoal/10 pb-4">
            ...
            <Link
              ...
              className="text-[10px] font-bold text-brand-gold hover:text-brand-charcoal uppercase tracking-widest transition-colors"
            >
              Finance
            </Link>
          </div>
          ...
          <div className="space-y-12">
            {data.gigs.length > 0 ? (
               data.gigs.slice(0, 3).map((gig) => {
                const totalPayout =
                   gig.payouts?.reduce(
                     (acc: number, val: Payout) => acc + Number(val.amount),
                     0,
                   ) || 0;
                ...
                return (
                  <div key={gig.id} className="space-y-4 group">
                    <div className="flex justify-between items-end">
                      <div>
                        <h4 className="text-xl font-bold text-brand-charcoal tracking-tight leading-none mb-2">
                          {gig.clients?.name}
                        </h4>
                        <p className="text-[10px] font-bold uppercase tracking-widest text-brand-charcoal/40">
                          Total: ₹{Number(gig.total_amount).toLocaleString()}
                        </p>
                      </div>
